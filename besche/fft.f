...
C     CONSTRUCTS THE DISCRETE FOURIER TRANSFORM OF Z1 (OR Z2)IN THE COOLEY-
...
C     CALLED IN F F T.
C     CARRIES OUT ONE STEP OF THE DISCRETE FAST FOURIER TRANSFORM.
      IMPLICIT NONE
      INTEGER AFTER,BEFORE,NOW,IA,IB,IN,J
      DOUBLE PRECISION ANGLE,RATIO,TWOPI
      DOUBLE COMPLEX ZIN(AFTER,BEFORE,NOW),ZOUT(AFTER,NOW,BEFORE),
     .      ARG,OMEGA,VALUE
      DATA TWOPI / 6.2831 85307 17958 64769 /
      ANGLE = TWOPI/DBLE(NOW*AFTER)
      OMEGA = DCMPLX(COS(ANGLE),-SIN(ANGLE))
      ARG = DCMPLX(1D0,0D0)
      DO 100 J=1,NOW
         DO 90 IA=1,AFTER
            DO 80 IB=1,BEFORE

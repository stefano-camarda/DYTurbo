      DOUBLE PRECISION FUNCTION dyALPHAS_lhapdf(Q)
...
c--- Note that the inputs AMZ and NLOOP are not used
      ...
      dyALPHAS_lhapdf=alphasPDF(Q)

      RETURN
      END
      
      DOUBLE PRECISION FUNCTION dyALPHAS_mcfm(Q,AMZ,NLOOP)
...
C---     DEL=SQRT(4*C2-C1**2)
      ...
      X1=dyALPHAS_mcfm(Q,AMZ,1)
      X2=dyALPHAS_mcfm(Q,AMZ,2)
      X3=dyALPHAS_mcfm(Q,AMZ,3)
      J=3
      IF (Q .GT. CMASS) J=4
      IF (Q .GT. BMASS) J=5
      EP=.001D0
      ...
      IF (NLOOP .EQ.1) THEN 
      BETA(J)=-B0(J)*X1**2
      DIFF1=(dyALPHAS_mcfm(QP,AMZ,1)-dyALPHAS_mcfm(QM,AMZ,1))
     .     /4d0/EP/BETA(J)
      ENDIF
      IF (NLOOP .EQ.2) THEN 
      BETA(J)=-B0(J)*X2**2*(1D0+C1(J)*X2)
      DIFF1=(dyALPHAS_mcfm(QP,AMZ,2)-dyALPHAS_mcfm(QM,AMZ,2))
     .     /4d0/EP/BETA(J)
      ENDIF
      IF (NLOOP .EQ.3) THEN 
      BETA(J)=-B0(J)*X3**2*(1D0+C1(J)*X3+C2(J)*X3**2)
      DIFF1=(dyALPHAS_mcfm(QP,AMZ,3)-dyALPHAS_mcfm(QM,AMZ,3))/4d0/EP
     .     /BETA(J)
      ENDIF

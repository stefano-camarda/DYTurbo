...
*
       IF ( IPAR .EQ. 1) THEN
         OPEN (92,FILE='usrinp.dat',STATUS='old')
         READ (92,*) M20 
         READ (92,*) ASI
         READ (92,*) MC2
         READ (92,*) MB2
       ...
       ELSE IF ( IPAR .EQ. 2) THEN
         CALL USRINP (PUV, PDV, PLM, PLP, PSM, PSP, PGL, M20, ASI,
...
*
       ...
       END IF
*
       IF (  (PLM(2) .LT.  1.D-3 ) .OR. (PLP(2) .LT. -0.85D0) 
     ,  .OR. (PSP(2) .LT. -0.85D0) .OR. (PGL(2) .LT. -0.85D0) ) THEN
...
*
       IF ( NFORM .EQ. 1 ) THEN
         CALL INPLMOM1 (PUV,PDV, PLM,PLP, PSM,PSP, PGL, IMOMIN,ISSIMP)
       ELSE IF ( NFORM .EQ. 2 ) THEN
         CALL INPLMOM2 (PUV,PDV, PLM,PLP, PSM,PSP, PGL, IMOMIN,ISSIMP)
       END IF
*
       IF (IVFNS .NE. 0) THEN

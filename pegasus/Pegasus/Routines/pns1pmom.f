...
*
*
* ..The subroutine  PNS1PMOM  for the NLO (alpha_s^2) polarized 
*    non-singlet QCD splitting functions  P1NS  in N-space for  
...
*
* ..The output common block  SPSUMS  is used by the routine PSG1PMOM.
*
* =====================================================================
*
*
...
*
       ...
       SSCHLM(KN) = SLC - SLV
       SSTR2M(KN) = ZETA(2) - DPSI (N1/2.,1)
       SSTR3M(KN) = 0.5 * DPSI (N1/2.,2) + ZETA(3)
       SSCHLP = SLC + SLV
       SSTR2P = ZETA(2) - DPSI (N2/2.,1)
       SSTR3P = 0.5 * DPSI (N2/2.,2) + ZETA(3)
*
* ---------------------------------------------------------------------
*
* ..The contributions to P1NS as given in Gonzalez-Arroyo et al. (1980)
*   (the polarized results are identical up to  `+' <-> `- = v') 
*
       PNPA = ( 16.* S1 * (2.* N + 1.) / (NS * N1S) +
     1          16.* (2.* S1 - 1./(N * N1)) * ( S2 - SSTR2M(KN) ) +
     2          64.* SSCHLM(KN) + 24.* S2 - 3. - 8.* SSTR3M(KN) -

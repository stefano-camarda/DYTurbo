...
bool old_plotter::IsInitialized(){return (h_qt!=0);};

void old_plotter::FillEvent(double p3[4], double p4[4], double wgt){
...
void old_plotter::FillRealDipole(double p3[4], double p4[4], double wgt, int nd){
...
void old_plotter::FillRealEvent(old_plotter::TermType term){
...
//void old_plotter::FillQuadrature(double int_val, double int_error){
//}

void old_plotter::FillResult(TermType term, double int_val, double int_error, double time){
...
//void old_plotter::CumulateResult(TermType term, double wgt){
    // TH2D * h = 0 ;
    // switch (term) {
    //     case Resum : h = qt_y_resum ; break;
    //     case CT    : h = qt_y_ct    ; break;
...
void old_plotter::SetPDF(int npdf){
...
void old_plotter::Merge(){
    //Dump();
}

void old_plotter::Dump(){
...
void old_plotter::Finalise(int worker){
    if (verbose){
        printf(" old_plotter:  histogram finalize\n");
        Dump();
    }
    bool isworker=false;
...
TH1 * old_plotter::clone_PDF(TH1*h,int npdf){
    TH1 *out = 0;
...
void old_plotter::addToBin(TH1*h, double int_val, double int_error){
    // Get bin
    // get the middle of bin
    double qt_val = ( phasespace::qtmax + phasespace::qtmin )/2.;
    double y_val  = ( phasespace::ymax  + phasespace::ymin  )/2.;
...
//
void old_plotter::calculate_kinematics(double p3[4], double p4[4])
{
  /**********************************************/
...
/*********************************************/
//Clean up begin
double old_plotter::calcQt(double p[4]){
    return  sqrt(p[0]*p[0]+p[1]*p[1]);
}
double old_plotter::calcY(double p[4]){
    return 0.5 *log((p[3]+p[2]) / (p[3]-p[2]));
}
double old_plotter::calcQ2(double p[4]){
    return p[3]*p[3] -p[0]*p[0] -p[1]*p[1] -p[2]*p[2];
}
double sqrt2=1.4142135623730951454746218587388284504413604736328125;
double old_plotter::Vplus(double p[4]){
    return (p[3] + p[2])        ;
}
double old_plotter::Vminus(double p[4]){
    return (p[3] - p[2])        ;
}
double old_plotter::calcCosThCS(double Q2,double qt,double p3[4],double p4[4]){
    double qz = p3[2]+p4[2];
    if (qz==0) return 0;
    double costh=0;
...
double old_plotter::calcPhiCS(double p3[4],double p4[4],double &phi_lep){
    // Collins-Sopper phi
...
void old_plotter::print_dipole(XsecPoint pt){
    printf("   ibin %d pt %f y %f wgt %f\n", pt.ibin, pt.qt, pt.y, pt.wgt );
}
void old_plotter::print_dipoleVec(std::vector<XsecPoint> vec ){
...
void old_plotter::rebin_vec(std::vector<double> & in_vec, std::vector<double> & out_vec, int N ){
...
old_plotter::old_plotter(){return;}
old_plotter::~old_plotter(){return;}
void old_plotter::Init(){return;}
bool old_plotter::IsInitialized(){return false;}
void old_plotter::FillEvent(double p3[4], double p4[4], double wgt){return;}
void old_plotter::FillRealDipole(double p3[4], double p4[4], double wgt,int nd){return;}
void old_plotter::FillRealEvent(TermType term ){return;}
//void old_plotter::FillQuadrature(double int_val, double int_error){return;}
void old_plotter::FillResult(TermType term, double int_val, double int_error, double time){return;}
//void old_plotter::CumulateResult(TermType term, double wgt){return;}
void old_plotter::SetPDF(int npdf){return;}
void old_plotter::Merge(){return;}
void old_plotter::Dump(){return;}
void old_plotter::Finalise(int worker){return;}
